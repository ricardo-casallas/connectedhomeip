...
public:
    ...
    virtual ~WindowActuator() = default;

    void DeltaSet(uint16_t delta);

    virtual void OpenLimitSet(uint16_t limit);
    virtual uint16_t OpenLimitGet(void);

    virtual void ClosedLimitSet(uint16_t limit);
    virtual uint16_t ClosedLimitGet(void);

    virtual void PositionSet(uint16_t position, PositionUnits units);
    virtual uint16_t PositionGet(PositionUnits units);

    virtual void NumberOfActuationsIncrement();
    virtual uint8_t NumberOfActuationsGet(void);

    virtual void TargetSet(uint16_t position, PositionUnits units);
    virtual uint16_t TargetGet(PositionUnits units);

    virtual void Stop(void) {};
    
protected:
    AttributeIds mAttributeIds;
    uint16_t mTarget = 0; // 0.01%
    uint8_t mStep = 0;
