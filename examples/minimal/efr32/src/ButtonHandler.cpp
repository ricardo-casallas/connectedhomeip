...
ButtonHandler::ButtonCallback_t ButtonHandler::sCallback = nullptr;
...
void ButtonHandler::Init(ButtonCallback_t callback)
{
    mCallback = callback;
...
void ButtonHandler::ButtonTimerCallback(AppTimer & timer, void * context)
{
    ...
    if (btn)
    {
        //bool is_pressed           = !GPIO_PinInGet(btn->mPort, btn->mPin);
        if(ButtonHandler::sInstance.mCallback)
        {
            ButtonHandler::sInstance.mCallback(*btn);
        }
