...
namespace example
{
    CHIP_ERROR WindowApp::Init()
    {
        ...
        ConfigStatus_t configStatus = { .operational = 1, .online = 1, .liftIsReversed = 0, .liftIsPA = 1, .tiltIsPA = 1, .liftIsEncoderControlled = 1, .tiltIsEncoderControlled = 1 };
        cover.ConfigStatusSet(configStatus);

        // Attribute: Id 10 OperationalStatus
        OperationalStatus_t operationalStatus = { .global = Stall, .lift = Stall, .tilt = Stall };
        ...
        Mode_t mode = { .motorDirReversed = 0, .calibrationMode = 1, .maintenanceMode = 1, .ledDisplay = 1 };
        cover.ModeSet(mode);

        // Attribute: Id 27 SafetyStatus (Optional)
        cover.SafetyStatusSet(0x00); // 0 is no issues;

        return CHIP_NO_ERROR;
    }


    CHIP_ERROR WindowApp::Main()
    {
        emberAfWindowCoveringClusterPrint("#### WindowApp::Main()");
    ...
    void WindowApp::OnLiftUp()
    {
        ...
        uint16_t percent = lift.PositionGet(WindowActuator::PositionUnits::Percentage);
        if(percent < 90) {
            percent += 10;
        }
        else {
        ...
        lift.PositionSet(percent, WindowActuator::PositionUnits::Percentage);
    }
    ...
    void WindowApp::OnTiltUp()
    {
        ...
        uint16_t percent = tilt.PositionGet(WindowActuator::PositionUnits::Percentage);
        if(percent < 90) {
            percent += 10;
        }
        else {
        ...
        tilt.PositionSet(percent, WindowActuator::PositionUnits::Percentage);
    }
    ...
    {
        WindowActuator &lift = WindowCover::Instance().Lift();
        uint16_t current = lift.PositionGet(WindowActuator::PositionUnits::Percentage100ths);
        uint16_t target = lift.TargetGet(WindowActuator::PositionUnits::Percentage100ths);
    ...
    {
        WindowActuator &lift = WindowCover::Instance().Tilt();
        uint16_t current = lift.PositionGet(WindowActuator::PositionUnits::Percentage100ths);
        uint16_t target = lift.TargetGet(WindowActuator::PositionUnits::Percentage100ths);
