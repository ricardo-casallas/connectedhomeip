...
#include <app/server/OnboardingCodesUtil.h> // for GetQRCode
...
Timer::Timer(const char *name, uint32_t timeoutInMs, Callback callback, void *context) :
    sample::Timer(name, timeoutInMs, callback, context)
{
...
// void Timer::Start(uint32_t timeoutInMs)
// {
//     if (xTimerIsTimerActive(mHandler))
//     {
//         EFR32_LOG("Timer already started!");
...
static const AppTask::Button::ButtonConfig sButtonsConfig[BSP_BUTTON_COUNT] = BSP_BUTTON_INIT;


AppTask::Button::Button(const char * name, Timer::Callback callback) :
    mTimer(name, APP_BUTTON_DEBOUNCE_PERIOD_MS, callback, this)
{
}


void AppTask::Button::Init(const Button::ButtonConfig &config)
{
    mPort = config.port;
    mPin = config.pin;
...
void AppTask::Button::OnInterrupt(uint8_t pin)
{
...
void AppTask::Button::OnUpTimeout(sample::Timer & timer)
{
    sInstance.PostEvent(sInstance.mButtonUp.mIsPressed ? AppEvent::UpPressed : AppEvent::UpReleased);
}


void AppTask::Button::OnDownTimeout(sample::Timer & timer)
{
    sInstance.PostEvent(sInstance.mButtonDown.mIsPressed ? AppEvent::DownPressed : AppEvent::DownReleased);
}
...
void AppTask::Run(void * params)
{
    AppTask::sInstance.Main();
}


AppTask::AppTask() :
    mButtonUp("Button:Up", Button::OnUpTimeout),
    mButtonDown("Button:Down", Button::OnDownTimeout),
...
CHIP_ERROR AppTask::Init()
{
    ...
    // Start App task.
    mHandle = xTaskCreateStatic(Run, APP_TASK_NAME, ArraySize(sAppStack), NULL, 1, sAppStack, &sAppTaskStruct);
    if (NULL == mHandle)
    {
    ...
    WindowActuator &lift = WindowCover::Instance().Lift();
    WindowActuator &tilt = WindowCover::Instance().Tilt();
    lift.OpenLimitSet(LIFT_OPEN_LIMIT);
    lift.ClosedLimitSet(LIFT_CLOSED_LIMIT);
    lift.PositionSet(LIFT_CLOSED_LIMIT, WindowActuator::PositionUnits::Value);
    tilt.OpenLimitSet(TILT_OPEN_LIMIT);
    tilt.ClosedLimitSet(TILT_CLOSED_LIMIT);
    tilt.PositionSet(TILT_CLOSED_LIMIT, WindowActuator::PositionUnits::Value);
...
sample::Timer *AppTask::CreateTimer(const char *name, uint32_t timeoutInMs, sample::Timer::Callback callback, void *context)
{
    return new Timer(name, timeoutInMs, callback, context);
}


void AppTask::DestroyTimer(sample::Timer *timer)
{
    delete timer;
}
...
void AppTask::ProcessEvents()
{
    AppEvent event = AppEvent::None;
...
void AppTask::OnProvisionedStateChanged(bool isProvisioned)
{
    UpdateLEDs();
    UpdateLCD();
}
...
void AppTask::OnConnectivityStateChanged(bool isConnected)
{
    UpdateLEDs();
}


void AppTask::OnBLEConnectionsChanged(bool haveConnections)
{
    UpdateLEDs();
}


void AppTask::OnCoverTypeChanged()
{
    sInstance.UpdateLCD();
}


void AppTask::OnLiftChanged()
{
    UpdateLCD();
}


void AppTask::OnTiltChanged()
{
    UpdateLCD();
}

void AppTask::OnTiltModeChanged()
{
    mIconTimer.Start();
    sInstance.UpdateLCD();
}
...
void AppTask::OnReset()
{
    ConfigurationMgr().InitiateFactoryReset();
}


void AppTask::OnIconTimeout(sample::Timer & timer)
{
    sInstance.UpdateLCD();
}
...
void AppTask::UpdateLEDs()
{
    ...
    else
    {
        ...
        if (AppEvent::None != mLiftAction || AppEvent::None != mTiltAction)
        {
...
void AppTask::UpdateLCD()
{
    ...
    if (mState.isThreadProvisioned)
    {
        ...
        uint16_t lift = cover.Lift().PositionGet(WindowActuator::PositionUnits::Value);
        uint16_t tilt = cover.Tilt().PositionGet(WindowActuator::PositionUnits::Value);
